 ...
 * </ol>
 * or constructor annotated with {@link SerializerCreator}
 * <ol>
...
public final class ConfigurableSerializer implements Serializer {
    private static final Map<Class<?>, McfClassSerializer> CACHE = new ConcurrentHashMap<>();
     ...
     */
    private <T> McfClassSerializer createClassMetadata(Class<T> type){
        ...
        return new McfClassSerializer(modules,serializer,deserializer);
    }

    /**
     * Return public constructor annotated with {@link SerializerCreator} or public no args constructor.
     * if no constructor is able to used, then throw runtime exception
    ...
    private static <T> Constructor<T> resolveConstructor(Class<T> mcfClass){
        Constructor<T> ctor;
        try{
            ctor = getCreatorConstructor(mcfClass);
     ...
     */
    @SuppressWarnings("unchecked")
    private static <T> Constructor<T> getCreatorConstructor(Class<T> tClass){
