...
class EncryptorAesGcmTest {
    ...
    void encrypt_ShouldNotReturnBlank(CredentialsEncryptorAesGcm encryptor) throws Exception{
    ...
    @ParameterizedTest
    @MethodSource("provider")
    void decrypt_ShouldNotReturnTheSameSecret(CredentialsEncryptorAesGcm encryptor) throws Exception{
        byte[] secret = secretBytes();
        String encrypted = encryptor.encrypt(secret);
    ...
    @ParameterizedTest
    @MethodSource("provider")
    void decrypt_shouldReturnSecret(CredentialsEncryptorAesGcm encryptor) throws Exception{
        byte[] secret = secretBytes();
        String encrypted = encryptor.encrypt(secret);
    ...
    public static Stream<CredentialsEncryptorAesGcm> provider() throws Exception{
        return Stream.of(
                new CredentialsEncryptorAesGcm(keyGen())
        );
    }
